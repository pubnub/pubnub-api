...
namespace PubnubMessaging
{
	public partial class Pubnub_MessagingSub : DialogViewController
	{
		...
		Dictionary<string, RectangleF> GetRectanglesForIphone ()
		{
			...
			int unsubX = spacingX;
			int unsubWidth = 150;
			dicRect.Add("unsub", new RectangleF (unsubX, row3Y, unsubWidth, buttonHeight));

			int unsubPresX = unsubX + unsubWidth + spacingX * 2;
			int unsubPresWidth = 150;
			dicRect.Add("unsubpres", new RectangleF (unsubPresX, row3Y, unsubPresWidth, buttonHeight));
		...
		void PublishAlertDismissed (object sender, UIButtonEventArgs e)
		{
			...
			if ((iav != null) && (!String.IsNullOrWhiteSpace (iav.EnteredText))) {
                ...
                {
				    pubnub.Publish<string> (channel.Trim(), iav.EnteredText, DisplayReturnMessage);
                }
			}
		}
		...
		public void DetailedHistory ()
        {
            ...
            string[] channels = Channel.Split (',');
            foreach (string channel in channels) {
                pubnub.DetailedHistory<string> (channel.Trim(), 100, DisplayReturnMessage);
            }
		}
		...
		public void HereNow ()
        {
            ...
            string[] channels = Channel.Split (',');
            foreach (string channel in channels) {
                pubnub.HereNow<string> (channel.Trim(), DisplayReturnMessage);
            }
		}
		...
		public void Unsub()
		{
            ...
            pubnub.Unsubscribe<string>(Channel, DisplayReturnMessage, DisplayConnectStatusMessage, DisplayConnectStatusMessage);
		}

		public void UnsubPresence()
		{
			Display("Running presence-unsubscribe");
            pubnub.PresenceUnsubscribe<string>(Channel, DisplayReturnMessage, DisplayConnectStatusMessage, DisplayConnectStatusMessage);
		}
