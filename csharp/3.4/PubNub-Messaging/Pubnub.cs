...
#endif
//TODO: Check with stakeholder whether to handle presence name containing "-pnpres"
namespace PubNubMessaging.Core
{
    ...
    public class Pubnub : INotifyPropertyChanged
    {
        ...
        bool overrideTcpKeepAlive = false;
        ...
        //static bool _multiChannelInternetStatus = false;
        //static int _multiChannelInternetRetry = 0;
        ...
        // Multiplex messages for Subscribe and Presence
        private ConcurrentDictionary<string, object> _multiChannelSubscribeMessage = new ConcurrentDictionary<string, object>();
                ...
                if (netState != null)
                {
                    ...
                    if (_channelInternetStatus[channel])
                    {
                        ...
                        switch (netState.Type)
                        {
                            ...
                            case ResponseType.Presence:
                                MultiChannelSubscribeRequest<T>(netState.Type, netState.Channels, netState.Timetoken, netState.Callback, netState.ConnectCallback, false);
                                break;
                            default:
                                break;
                        }
                    ...
                    else if (_channelInternetRetry[channel] >= pubnubNetworkCheckRetries)
                    {
                        ...
                        switch (netState.Type)
                        {
                            case ResponseType.Subscribe:
                            case ResponseType.Presence:
                                MultiplexExceptionHandler(netState.Type, netState.Channels, netState.Callback, netState.ConnectCallback, true);
                                break;
                            default:
                                break;
                        }
        ...
        private void TerminatePendingWebRequest<T>(RequestState<T> state)
        {
            ...
            else
            {
                ...
                foreach (string key in keyCollection)
                {
                    ...
                    {
                        TerminateHeartbeatTimer(currentRequest.RequestUri);
        ...
        private List<object> DecodeMessage123(List<object> message, ResponseType type)
        {
            ...
            if (rawChannels.Length > 0)
            {
                ...
                for (int index = 0; index < rawChannels.Length; index++)
                {
                    if (rawChannels[index].Trim().Length > 0)
                    {
                        ...
                        {
                            List<object> result = new List<object>();
                            string jsonString = string.Format("[0, \"{0} already subscribed\"]",channelName);
                            result = (List<object>)JsonConvert.DeserializeObject<List<object>>(jsonString);
            ...
            if (validChannels.Count > 0)
            {
                ...
                if (currentChannels != null && currentChannels.Length > 0)
                {
                    ...
                    if (_channelRequest.ContainsKey(multiChannelName))
                    {
                        ...
                        object multiChannelSubscribeMessage;
                        bool removedMessageStatus = _multiChannelSubscribeMessage.TryRemove(multiChannelName, out multiChannelSubscribeMessage); //Clear the dictionary received for last sub msg
                    }
                    else
                    {
        ...
        private void MultiChannelUnSubscribeInit<T>(ResponseType type, string channel, Action<T> userCallback, Action<T> connectCallback, Action<T> disconnectCallback)
        {
            ...
            if (rawChannels.Length > 0)
            {
                for (int index = 0; index < rawChannels.Length; index++)
                {
                    if (rawChannels[index].Trim().Length > 0)
                    {
                        ...
                        {
                            List<object> result = new List<object>();
                            string jsonString = string.Format("[0, \"{0} not subscribed\"]",channelName.Replace("-pnpres", ""));
                            result = (List<object>)JsonConvert.DeserializeObject<List<object>>(jsonString);
                            result.Add(channelName.Replace("-pnpres", ""));
                            LoggingMethod.WriteToLog(string.Format("DateTime {0}, JSON response={1}", DateTime.Now.ToString(), jsonString), LoggingMethod.LevelInfo);
                            GoToCallback<T>(result, userCallback);
            ...
            if (validChannels.Count > 0)
            {
                ...
                for (int index = 0; index < validChannels.Count; index++)
                {
                    ...
                    {
                        List<object> result = new List<object>();
                        string jsonString = string.Format("[1, \"Unsubscribed from {0}\"]", channelToBeRemoved.Replace("-pnpres", ""));
                        result = (List<object>)JsonConvert.DeserializeObject<List<object>>(jsonString);
                    ...
                    {
                        List<object> result = new List<object>();
                        string jsonString = string.Format("[0, \"Error unsubscribing from {1}\"])", channelToBeRemoved.Replace("-pnpres", ""));
                        result = (List<object>)JsonConvert.DeserializeObject<List<object>>(jsonString);
                        result.Add(channelToBeRemoved.Replace("-pnpres", ""));
                        LoggingMethod.WriteToLog(string.Format("DateTime {0}, JSON response={1}", DateTime.Now.ToString(), jsonString), LoggingMethod.LevelInfo);
                        GoToCallback<T>(result, userCallback);
        ...
        void OnPubnubWebRequestTimeout(object state, bool timeout)
        {
            if (timeout)
            {
                RequestState<object> currentState = state as RequestState<object>;
                if (currentState != null)
                {
                    ...
                    if (request != null)
                    {
                        ...
                        LoggingMethod.WriteToLog(string.Format("DateTime: {0}, OnPubnubWebRequestTimeout: client request timeout reached.Request aborted for channel = {1}", DateTime.Now.ToString(), string.Join(",",currentState.Channels)), LoggingMethod.LevelInfo);
                        request.Abort();
                    }
                }
                else
        ...
        void OnPubnubHeartBeatTimeoutCallback(Object heartbeatState)
        {
            ...
            RequestState<object> currentState = heartbeatState as RequestState<object>;
            if (currentState != null)
            {
        ...
        private void MultiplexInternalCallback<T>(ResponseType type, object multiplexResult, Action<T> userCallback, Action<T> connectCallback)
        {
            ...
            //TODO:
            //if (!_multiChannelSubscribe.ContainsKey(channels))
            //{
            //    LoggingMethod.WriteToLog(string.Format("DateTime {0}, Due to Unsubscribe, further re-subscription was stopped for channel {1}", DateTime.Now.ToString(), channels.ToString()), LoggingMethod.LevelInfo);
            //    return;
            //}
        ...
        //private void PresenceInternalCallback123<T>(object presenceResult, Action<T> userCallback)
        //{
        ...
        //    if (message != null && message.Count >= 3)
        //    {
        //        PresenceRequest123<T>(channelName, (object)message[1], userCallback, false);
        //    }
        //}
        ...
        private void MultiChannelSubscribeRequest<T>(ResponseType type, string[] channels, object timetoken, Action<T> userCallback, Action<T> connectCallback, bool reconnect)
        {
            ...
            //{
            //    _multiChannelSubscribe.AddOrUpdate(channel, Convert.ToInt64(timetoken.ToString()), (key, oldValue) => Convert.ToInt64(timetoken.ToString()));
            //}
            ...
            if (_channelInternetStatus.ContainsKey(multiChannel) && (!_channelInternetStatus[multiChannel]) && _pubnetSystemActive)
            {
                if (_channelInternetRetry.ContainsKey(multiChannel) && (_channelInternetRetry[multiChannel] >= pubnubNetworkCheckRetries))
                {
                    ...
                    MultiplexExceptionHandler<T>(type,channels, userCallback, connectCallback, true);
                    return;
                }
            ...
            try
            {
                //string[] channelsSubscribed = _multiChannelSubscribe.Keys.ToArray<string>();
                ...
                pubnubRequestState.ConnectCallback = connectCallback;// as Action<object>;
                pubnubRequestState.UserCallback = userCallback;// as Action<object>;
        ...
        /// <param name="userCallback"></param>
        /// <param name="reconnect"></param>
        //private void PresenceRequest123<T>(string channel, object timetoken, Action<T> userCallback, bool reconnect)
        //{
        //    //Exit if the channel is unsubscribed
        ...
        //            PresenceExceptionHandler123<T>(channel, userCallback, true);
        //            return;
        //        }
        ...
        //    {
        //        // Build URL
        //        Uri request = BuildPresenceRequest123(channel, timetoken);
        ...
        //        this.PresenceRequest123<T>(channel, timetoken, userCallback, false);
        //    }
        //}
       ...
       /// <returns></returns>
            ...
            try
            {
                ...
                IAsyncResult asyncResult = request.BeginGetResponse(new AsyncCallback((asynchronousResult) =>
                {
                    ...
                    using (StreamReader streamReader = new StreamReader(asyncWebResponse.GetResponseStream()))
                    {
                        ...
                        result = WrapResultBasedOnResponseType(type, jsonString, new string[] {channelName}, false);
                    }
                }), request
        ...
        private bool UrlProcessRequest<T>(Uri requestUri, RequestState<T> pubnubRequestState)
        {
...
#endif
                //TODO: Fix multi channel arrary issue
                //if ((!_multiChannelSubscribe.ContainsKey(channels) && type == ResponseType.Subscribe)
                //    || (!_channelPresence.ContainsKey(channels) && type == ResponseType.Presence))
                //{
                //    LoggingMethod.WriteToLog(string.Format("DateTime {0}, Due to Unsubscribe, request aborted for channel={1}", DateTime.Now.ToString(), channels), LoggingMethod.LevelInfo);
                //    request.Abort();
                //}
...
#else
                ThreadPool.RegisterWaitForSingleObject(asyncResult.AsyncWaitHandle, new WaitOrTimerCallback(OnPubnubWebRequestTimeout), pubnubRequestState, GetTimeoutInSecondsForResponseType(pubnubRequestState.Type) * 1000, true);
#endif
                return true;
            }
            catch (System.Exception ex)
            {
                ...
                UrlRequestCommonExceptionHandler<T>(pubnubRequestState.Type, pubnubRequestState.Channels, false, pubnubRequestState.UserCallback, pubnubRequestState.ConnectCallback);
                return false;
            }
        }
        ...
        private void UrlProcessResponseCallback<T>(IAsyncResult asynchronousResult)
        {
            ...
            try
            {
                ...
                if (asyncWebRequest != null)
                {
                    using (PubnubWebResponse asyncWebResponse = (PubnubWebResponse)asyncWebRequest.EndGetResponse(asynchronousResult))
                    {
                        ...
                        using (StreamReader streamReader = new StreamReader(asyncWebResponse.GetResponseStream()))
                        {
                            ...
                            //if (asynchRequestState.Type == ResponseType.Subscribe && jsonString.IndexOf("channel(",0) == 0)
                            //{
                            //    jsonString = jsonString.Replace("channel(", "").Replace(")", "");
                            //}

                            result = WrapResultBasedOnResponseType(asynchRequestState.Type, jsonString, asynchRequestState.Channels, asynchRequestState.Reconnect);
                        }
                ...
                switch (asynchRequestState.Type)
                {
                    ...
                    case ResponseType.Presence:
                        ...
                        break;
                        ////PresenceInternalCallback<T>(result, asynchRequestState.UserCallback);
                        //break;
                    default:
                        break;
                }
            ...
            catch (WebException webEx)
            {
                ...
                if (overrideTcpKeepAlive)
                {
                    TerminateHeartbeatTimer(asynchRequestState.Request.RequestUri);
                }

                if (asynchRequestState.Response != null)
                {
                    asynchRequestState.Response.Close();
                    asynchRequestState.Request.Abort();
                }
                    ...
                    ) && (!overrideTcpKeepAlive))
                {
                        ...
                        && (asynchRequestState.Type == ResponseType.Subscribe || asynchRequestState.Type == ResponseType.Presence))
                    {
                        ...
                        {
                            _channelInternetRetry.AddOrUpdate(channel, 1, (key, oldValue) => oldValue + 1);
                            LoggingMethod.WriteToLog(string.Format("DateTime {0} {1} channel = {2} _urlRequest - Internet connection retry {3} of {4}", DateTime.Now.ToString(), asynchRequestState.Type, asynchRequestState.Channels, _channelInternetRetry[channel], pubnubNetworkCheckRetries), LoggingMethod.LevelInfo);
                        }
...
#endif
                UrlRequestCommonExceptionHandler<T>(asynchRequestState.Type, asynchRequestState.Channels, asynchRequestState.Timeout, asynchRequestState.UserCallback, asynchRequestState.ConnectCallback);
            }
            catch (Exception ex)
            {
                if (asynchRequestState.Response != null)
        ...
        private Uri BuildSubscribeRequest123(string channel, object timetoken)
        {
            List<string> url = new List<string>();
            url.Add("subscribe");
            url.Add(this.subscribeKey);
        ...
        private Uri BuildPresenceRequest123(string channel, object timetoken)
        {
            List<string> url = new List<string>();
            url.Add("subscribe");
            url.Add(this.subscribeKey);
        ...
        {
            RequestState<object> currentState = requestState as RequestState<object>;
        ...
        private void UrlRequestCommonExceptionHandler<T>(ResponseType type, string[] channels, bool requestTimeout, Action<T> userCallback, Action<T> connectCallback)
        {
            if (type == ResponseType.Subscribe || type == ResponseType.Presence)
            {
                MultiplexExceptionHandler<T>(type, channels, userCallback, connectCallback, false);
            }
        ...
        private void RemoveChannelRequest(string[] channels)
        {
            //TODO:
            //if (_channelRequest.ContainsKey(channelName))
            //{
            //    RequestState<object> currentRequest = _channelRequest[channelName];
        ...
        private void MultiplexExceptionHandler<T>(ResponseType type, string[] channels, Action<T> userCallback, Action<T> connectCallback, bool reconnectTried)
        {
            string channel = "";
            if (channels != null)
            {
            ...
            else
            {
                ...
                if (_multiChannelSubscribeMessage.ContainsKey(channel))
                {
                    lastResult = _multiChannelSubscribeMessage[channel] as List<object>;
                }
        ...
        //private void PresenceExceptionHandler123<T>(string channelName, Action<T> userCallback, bool reconnectTry)
        //{
        //    if (reconnectTry)
        //    {
        ...
        //        PresenceInternalCallback123<T>(result, userCallback);
        //    }
        //}
        ...
        /// <param name="reconnect"></param>
        /// <returns></returns>
        private List<object> WrapResultBasedOnResponseType(ResponseType type, string jsonString, string[] channels, bool reconnect)
        {
            List<object> result = new List<object>();
            ...
            switch (type)
            {
                ...
                case ResponseType.Presence:
                    ...
                    //TODO: Fix multi channel arrary issue
                    _multiChannelSubscribeMessage.AddOrUpdate(string.Join(",",channels), result, (key, oldValue) =>
                    {
                        if (reconnect)
                        {
