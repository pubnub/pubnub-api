            ...
            try
            {
                ...
                IAsyncResult asyncResult = request.BeginGetResponse(new AsyncCallback((asynchronousResult) =>
               {
                    ...
                    using (StreamReader streamReader = new StreamReader(aResponse.GetResponseStream()))
                    {
                        ...
                        if (result.Count != 0)
                        {
                            if (result[0] is List<object>)
                            {
                                this.ReturnMessage = result[0];
                                //foreach (object message in (List<object>)result[0])
                                //{
                        ...
                        switch (type)
                        {
                            ...
                            case ResponseType.DetailedHistory:
                                if (this.CIPHER_KEY.Length > 0)
                                {
                                    ...
                                    foreach (object message in result[0] as List<object>)
                                    {
                                        historyDecrypted.Add(aes.decrypt(message.ToString()));
                                    }
                                ...
                                {
                                    DetailedHistory = (List<object>)(result[0]);
                                }
                                break;
        ...
        public string EncryptOrDecrypt(bool type, string plainStr)
        {
            try
            {
                ...
                else
                {
                    ...
                    //ASCIIEncoding.UTF8.GetString(decrypto.TransformFinalBlock(encryptedBytes, 0, encryptedBytes.Length)); !!!
                    var data = decrypto.TransformFinalBlock(encryptedBytes, 0, encryptedBytes.Length);
