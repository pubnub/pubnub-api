...
a);for(a=a.firstChild;a;){k(a,b);a=a.nextSibling}}var u=/\${([\w\-]+)}/g;function z(a,b,c){function d(l){if(!l)l=window.event;if(!c(l)){l.cancelBubble=true;l.returnValue=false;if(l.stopPropagation){l.stopPropagation();l.preventDefault()}}}if(b.addEventListener)b.addEventListener(a,d,false);else if(b.attachEvent)b.attachEvent("on"+a,d);else b["on"+a]=d}function n(){return v("head")[0]}function g(a,b){if(!a)return"";var c=[],d="";for(d in a)c.push(escape(d)+"="+escape(a[d]));return(b.indexOf("?")===
-1?"?":"&")+c.join("&")}function i(a){return document.createElement(a)}function f(a){var b=i("script"),c=r(),d=100,l=setTimeout(function(){t("timeout")},a.timeout||3E4),y=a.data||{},m=a.b||w(),p=a.c||w();function s(){setTimeout(function(){d*=2;try{var q=n(),A=q.firstChild;A?q.insertBefore(b,A):q.appendChild(b)}catch(B){s()}},d)}function t(q){clearTimeout(l);if(b){q&&m.call(q,b,unescape(b.src));b.onerror=null;try{n().removeChild(b)}catch(A){}}}b.async="async";this[c]=function(q){if(!q)return t("no data");
p.call(b,q);this[c]=null;t(0)};z("error",b,function(){t("error")});y.unique=c;b.src=a.url+g(y,a.url);s();return t}var h="http://{{ORIGIN}}/",e={history:function(a,b){b=b||w();a.limit=a.limit||10;if(!a.channel)return j("Must Specify a Channel");a.channel=e.d+"/"+a.channel;f({url:h+"pubnub-history",data:a,c:function(c){b(c.messages)},b:function(c){j(c)}})},time:function(a){f({url:h+"pubnub-time",c:function(b){a(b.time)},b:function(){a(0)}})},uuid:function(a){f({url:h+"pubnub-uuid",c:function(b){a(b.uuid)},
b:function(){a(0)}})},publish:function(a,b){b=b||w()||a.callback;var c=a.message;if(!c)return j("Must Specify a Message");if(!a.channel)return j("Must Specify a Channel");c=JSON.stringify(c);if(c.length>1700)return j("Message exceeded limit 1700");if(!e.g)return j("Missing Publish Key");a.publish_key=e.g;a.channel=e.d+"/"+a.channel;a.message=c;f({url:h+"pubnub-publish",data:a,c:function(d){b(d)},b:function(d){b(d)}})},unsubscribe:function(a){a=e.d+"/"+a.channel;if(a in e.a){e.a[a].disabled=1;e.a[a].e=
0;e.a[a].f&&e.a[a].f(0)}},subscribe:function(a,b){function c(){setTimeout(function(){y*=2;f({url:h+"pubnub-subscribe",data:a,c:function(p){p&&p.server?d(p.server):c()},b:function(){c()}})},y)}function d(p){if(!(m in e.a&&e.a[m].disabled))e.a[m].f=f({url:"http://"+p,data:{channel:m,timetoken:l},c:function(s){l=s.timetoken;d(p);s.messages[0]!="xdr.timeout"&&o(s.messages,function(t){b(t)})},b:function(s){s=="timeout"?d(p):c()}})}if(!a.channel)return j("Must Specify a Channel");b=b||a.callback;if(!b)return j("Must Specify a Callback");
if(!e.d)return j("Missing Subscribe Key");var l=0,y=100,m=a.channel=e.d+"/"+a.channel;m in e.a||(e.a[m]={});if(e.a[m].e)return j("Already Connected to this Channel: "+m);e.a[m].disabled=0;e.a[m].e=1;c()},g:"{{pub_key}}",d:"{{sub_key}}",h:"{{app_key}}",a:{},each:o,map:function(a,b){var c=[];o(a||[],function(d,l){c.push(b(d,l))});return c},css:function(a,b){for(var c in b)if(b.hasOwnProperty(c))try{a.style[c]=b[c]+("|width|height|top|left|".indexOf(c)>0?"px":"")}catch(d){}},$:function(a){return document.getElementById(a)},
