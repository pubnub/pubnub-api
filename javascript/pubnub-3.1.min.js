...
t();f({c:h,url:["http"+o+"://pubnub-prod.appspot.com/uuid?callback="+h],b:function(i){e(i[0])},a:function(){e(0)}})},publish:function(e,h){h=h||e.callback||y();var i=e.message,l=e.channel,r=t();if(!i)return g("Missing Message");if(!l)return g("Missing Channel");if(!c)return g("Missing Publish Key");i=JSON.stringify(i);if(i.length>1800)return g("Message Too Big");f({c:r,b:function(v){h(v)},a:function(){h([0,"Disconnected"])},url:[k,"publish",c,d,0,n(l),r,n(i)]})},unsubscribe:function(e){e=e.channel;
if(e in b){b[e].d=0;b[e].e&&b[e].e(0)}},subscribe:function(e,h){function i(){var L=t();if(b[l].d)b[l].e=f({c:L,url:[k,"subscribe",d,n(l),L,r],a:function(){setTimeout(i,1E3);v()},b:function(M){if(b[l].d){r=M[1];setTimeout(i,10);w(M[0],function(R){h(R)})}}})}if(!G)return N.push([e,h,D]);var l=e.channel;h=h||e.callback;var r=0,v=e.error||y();if(!l)return g("Missing Channel");if(!h)return g("Missing Callback");if(!d)return g("Missing Subscribe Key");l in b||(b[l]={});if(b[l].d)return g("Already Connected");
b[l].d=1;i()},each:w,map:u,css:s,$:z,create:B,bind:q,supplant:p,head:j,search:C,attr:m,now:A,init:E};return D}function A(){return"x"+ ++S+""+ +new Date}function z(a){return document.getElementById(a)}function g(a){console.log(a)}function C(a){var b=[];w(a.split(/\s+/),function(c){w(document.getElementsByTagName(c),function(d){b.push(d)})});return b}function w(a,b){if(a&&b)if(typeof a[0]!="undefined")for(var c=0,d=a.length;c<d;)b.call(a[c],a[c],c++);else for(c in a)a.hasOwnProperty&&a.hasOwnProperty(c)&&
b.call(a[c],c,a[c])}function u(a,b){var c=[];w(a||[],function(d,o){c.push(b(d,o))});return c}function p(a,b){return a.replace(T,function(c,d){return""+b[d]||""})}function q(a,b,c){w(a.split(","),function(d){function o(k){if(!k)k=window.event;if(!c(k)){k.cancelBubble=true;k.returnValue=false;k.preventDefault&&k.preventDefault();k.stopPropagation&&k.stopPropagation()}}if(b.addEventListener)b.addEventListener(d,o,false);else if(b.attachEvent)b.attachEvent("on"+d,o);else b["on"+d]=o})}function j(){return C("head")[0]}
...
l){if(!k){k=1;i||h(l);c.onerror=null;clearTimeout(D);setTimeout(function(){i&&e();var r=z(o),v=r&&r.parentNode;v&&v.removeChild(r)},1E3)}}if(H||x())return U(a);var c=B("script"),d=a.c,o=A(),k=0,D=setTimeout(function(){b(1)},I),e=a.a||y(),h=a.b||y();window[d]=function(i){b(0,i)};c[O]=O;c.onerror=function(){b(1)};c.src=a.url.join(P);m(c,"id",o);j().appendChild(c);return b}function U(a){function b(l){if(!o){o=1;clearTimeout(D);if(d){d.onerror=d.onload=null;d.abort&&d.abort();d=null}l&&e()}}function c(){if(!k){k=
...
0)return 0;var a={id:x.id++,send:y(),abort:function(){a.id={}},open:function(b,c){x[a.id]=a;J.get(a.id,c)}};return a}window.console||(window.console=window.console||{});console.log||(console.log=(window.opera||{}).postError||y());var S=1,T=/\$?{([\w\-]+)}/g,O="async",P="/",I=14E4,H=1,F=z("pubnub"),G=0,N=[],K=E({publish_key:m(F,"pub-key"),subscribe_key:m(F,"sub-key"),ssl:m(F,"ssl")=="on",origin:m(F,"origin")});q("load",window,function(){setTimeout(Q,1E3)});s(F,{position:"absolute",top:-1});F.innerHTML=
"<object id=pubnubs type=application/x-shockwave-flash width=1 height=1 data=http://pubnub.s3.amazonaws.com/pubnub.swf><param name=movie value=http://pubnub.s3.amazonaws.com/pubnub.swf /><param name=allowscriptaccess value=always /></object>";var J=z("pubnubs")||{},V=setInterval(function(){J.get&&Q(V)},100);K.rdx=function(a,b){if(!b)return x[a].onerror();x[a].responseText=unescape(b);x[a].onload()};x.id=1E3;window.jQuery&&(window.jQuery.PUBNUB=K);window.PUBNUB=K}();
