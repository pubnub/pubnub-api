...
namespace PubNubMessaging.Core
{
	...
	public class MainActivity : Activity
	{
		...
		public void DetailedHistory ()
		{
			...
			string[] channels = channel.Split (',');
			foreach (string channelToCall in channels) {
				pubnub.DetailedHistory<string> (channelToCall.Trim(), 100, DisplayReturnMessage);
			}
		}
		...
		public void HereNow ()
		{
			...
			string[] channels = channel.Split (',');
			foreach (string channelToCall in channels) {
				pubnub.HereNow<string> (channelToCall.Trim(), DisplayReturnMessage);
			}
		}
		...
		public void Unsub()
		{
			...
			pubnub.Unsubscribe<string>(channel, DisplayReturnMessage, DisplayReturnMessage, DisplayReturnMessage);
		}
		
		public void UnsubPresence()
		{
			Display("Running presence-unsubscribe");
			pubnub.PresenceUnsubscribe<string>(channel, DisplayReturnMessage, DisplayReturnMessage, DisplayReturnMessage);
		}
