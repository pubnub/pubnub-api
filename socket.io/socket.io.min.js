...
(function(){function t(b){f.each(g,function(a){var c=i[a][b];if(c.connected){c.connected=false;c.socket.user_count--;f.events.fire(a+"leave",c)}})}function w(b,a){var c=m(b);return"password"in c&&c.password&&sjcl.encrypt(c.password,JSON.stringify(a)+"")||a}function x(b,a){var c=m(b);if(!c.password)return a;try{return JSON.parse(sjcl.decrypt(c.password,a))}catch(j){return null}}function p(b,a,c,j,s){var n=m(a).p;console.log("PUBNUB->",n,a,c,b);n.publish({channel:n.channel,message:{name:b,ns:a,data:w(a,
c||{}),uuid:q,geo:n.location||[0,0]},callback:function(l){if(l[0])return(s||function(){})(l);var h=(j||500)*2;setTimeout(function(){p(b,a,c,h,s)},h>5500?5500:h)}})}function u(b){b=b||function(){};navigator&&navigator.geolocation&&navigator.geolocation.getCurrentPosition(function(a){f.location=[a.coords.latitude,a.coords.longitude];b(f.location)})||b([0,0])}function m(b){b=b;return g[b]||function(){return g[b]={namespace:b,connected:false,users:i[b]={},user_count:1,get_user_list:function(){return g[b].users},
get_user_count:function(){return g[b].user_count},emit:function(a,c,j){p(a,b,c,0,j)},send:function(a,c){p("message",b,a,0,c)},on:function(a,c){if(typeof a==="string")f.events.bind(b+a,c);else typeof a==="object"&&f.each(a,function(j){f.events.bind(b+j,c)})},disconnect:function(){delete g[b]}}}()}var f=PUBNUB,q=PUBNUB.db.get("uuid")||f.uuid(function(b){PUBNUB.db.set("uuid",q=b)}),g={},i={},v=window.io={connected:{},connect:function(b,a){var c=(b+"////").split("/");a=a||{};var j=a.user||{},s="presence"in
a?a.presence:true,n=c[2],l=(c[3]||"standard")+"-"+a.channel,h=m(l);h.password="sjcl"in window&&a.password;if(l in v.connected)return h;v.connected[l]=true;a.geo&&setInterval(u,15E3)&&u();a.origin=n;var d=h.p=PUBNUB.init(a);d.disconnected=0;d.channel=h.channel=a.channel;d.subscribe({channel:d.channel,disconnect:function(){if(!d.disconnected){d.disconnected=1;d.each(g,function(e){d.events.fire(e+"disconnect",{})})}},reconnect:function(){d.disconnected=0},connect:function(){d.disconnected=0;d.each(g,
function(e){if(!m(e).connected){m(e).connected=true;d.events.fire(e+"connect",{})}})},callback:function(e){d.disconnected&&d.each(g,function(k){d.events.fire(k+"reconnect",{})});d.disconnected=0;var o=x(e.ns,e.data);e.ns in g&&o&&d.events.fire(e.ns+e.name,o);if(!(!e.uuid||e.uuid===q)){e.name==="ping"&&d.each(o.nss,function(k){i[k]=i[k]||{};var r=i[k][e.uuid]=i[k][e.uuid]||{geo:e.geo||[0,0],uuid:e.uuid,last:+new Date,socket:h,namespace:k,connected:false,slot:h.user_count++};r.last=+new Date;r.data=
o.cuser;if(!r.connected){r.connected=true;d.events.fire(k+"join",r)}});e.name==="user-disconnect"&&t(e.uuid)}}});if(s)d.tcpKeepAlive=setInterval(d.updater(function(){var e=d.map(g,function(o){return o});p("ping",l,{nss:e,cuser:j})},3E3),500);return h}};setInterval(function(){f.each(i,function(b){f.each(i[b],function(a){var c=i[b][a];+new Date-c.last<5500||a===q||t(c.uuid)})})},1E3);typeof window!=="undefined"&&f.bind("beforeunload",window,function(){p("user-disconnect","",q);return true})})();
